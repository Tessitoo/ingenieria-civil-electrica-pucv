...
function Ramo(nombre, sigla, creditos, sector, prer = [], id, colorBySector) {
  ...
  // Dibuja el ramo en el SVG
  this.draw = function (canvas, posX, posY, scaleX, scaleY) {
    ramo = canvas
      .append("g")
      .attr("id", self.sigla)
      .attr("data-sector", self.sector); // para saber de qué tipo es este ramo

    var sizeX = 100 * scaleX,
      sizeY = 100 * scaleY;
    var graybar = sizeY / 5;

    // Fondo principal del ramo
    ramo
      .append("rect")
      .attr("x", posX)
      .attr("y", posY)
      ...
      .attr("height", sizeY)
      .attr("fill", colorBySector[self.sector][0])
      .attr("class", "ramo-base"); // clase para luego cambiar color
    ...
    ramo
      ...
      .attr("font-family", "sans-serif")
      .attr("fill", function () {
        if (self.sector != "IS") return "white";
        return "#6D6E71";
      })
      .attr("font-size", 13)
    ...
    ramo
      ...
      .attr("font-size", function () {
        if (scaleX < 0.59) return 9;
        return 12;
      });

    // Capa transparente para bloquear si no tiene prerrequisitos cumplidos
    ramo
      .append("rect")
      .attr("x", posX)
      .attr("y", posY)
    ...
    cross
      ...
      .attr(
        "d",
        "M" + posX + "," + posY + "L" + (posX + sizeX * 1.1) + "," + (posY + sizeY)
      )
      .attr("stroke", "#550000")
      .attr("stroke-width", 9);

    // Circulito de ID del ramo
    ramo
      .append("circle")
    ...
    ramo
      .append("text")
      .attr("x", function () {
        if (self.id > 9) return posX + sizeX * 1.2 - 10;
    ...
    self.prer.forEach(function (p) {
      ...
      const prereqRamo = all_ramos[p];
      const prereqSector = prereqRamo ? prereqRamo.sector : self.sector;

      ramo
        .append("circle")
        .attr("class", "prer-circle")
        .attr("data-sector", prereqSector)
        .attr("cx", posX + r + c_x + variantX)
        .attr("cy", posY + sizeY - graybar / 2)
        .attr("r", r)
        .attr("fill", colorBySector[prereqSector][0])
        .attr("stroke", "white");
    ...
    // Click en el ramo → aprobar / desaprobar
    ramo.on("click", self.approveRamo);
  };

  // Marca / desmarca el ramo como aprobado
  this.approveRamo = function () {
    if (!approved) {
      d3.select("#" + self.sigla)
        .select(".cross")
        .transition()
        .delay(20)
        .attr("opacity", "1");
  ...
  // Necesario para saber si está aprobado
